...
The `home-routes` handler in the `clojure-photo-bank.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-photo-bank.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clojure-photo-bank.handler` namespace:
...
Request middleware functions are located under the `clojure-photo-bank.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-photo-bank.dev-middleware` namespace found in
